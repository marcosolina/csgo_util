...
using DemoInfoPlayer = DemoInfo.Player;
using DemoInfoTeam = DemoInfo.Team;
...
namespace DemParser.Analyzer
{
    public abstract class DemoAnalyzer
    {
        ...
        public Dictionary<DemoInfoPlayer, int> KillsThisRound { get; set; } = new Dictionary<DemoInfoPlayer, int>();
        ...
        protected void HandleTickDone(object sender, TickDoneEventArgs e)
        {
            ...
            if (AnalyzeFlashbang)
            {
                ...
                {
                    foreach (DemoInfoPlayer player in Parser.PlayingParticipants)
                    {
        ...
        protected void HandleRoundOfficiallyEnd(object sender, RoundOfficiallyEndedEventArgs e)
        {
            ...
            if (!IsRoundEndOccured)
            {
                ...
                if (teamCt.Score < Parser.CTScore)
                {
                    ...
                    {
                        Winner = DemoInfoTeam.CounterTerrorist,
                    });
                }
                else
                {
        ...
        protected void HandleWeaponFired(object sender, WeaponFiredEventArgs e)
        {
            ...
            if (!IsFirstShotOccured)
            {
                ...
                foreach (DemoInfoPlayer pl in Parser.PlayingParticipants)
                {
        ...
        protected void HandleFlashNadeExploded(object sender, FlashEventArgs e)
        {
            ...
            {
                foreach (DemoInfoPlayer pl in e.FlashedPlayers)
                {
                    Player player = Demo.Players.FirstOrDefault(p => p.SteamId == pl.SteamID);
                    if (player != null)
                    {
        ...
        {
            if (e.Swapped == null || e.Swapped.SteamID == 0 || e.NewTeam == DemoInfoTeam.Spectate)
            {
                return;
            }
        ...
        {
            foreach (KeyValuePair<DemoInfoPlayer, int> pair in KillsThisRound)
            {
        ...
        protected void CreateNewRound(bool force = false)
        {
            ...
            foreach (Player pl in Demo.Players)
            {
                ...
                DemoInfoPlayer player = Parser.PlayingParticipants.FirstOrDefault(p => p.SteamID == pl.SteamId);
                if (player != null)
                {
        ...
        protected void ProcessClutches()
        {
            ...
            if (_playerInClutch1 == null && terroristAliveCount == 1)
            {
                ...
                DemoInfoPlayer pl = Parser.PlayingParticipants.FirstOrDefault(p => p.Team.ToSide() == Side.Terrorist && p.IsAlive);
                if (pl == null)
                {
                    return;
                }
            ...
            {
                DemoInfoPlayer pl = Parser.PlayingParticipants.FirstOrDefault(p => p.Team.ToSide() == Side.CounterTerrorist && p.IsAlive);
                if (pl == null)
                {
                    return;
                }
            ...
            {
                DemoInfoPlayer player1 = Parser.PlayingParticipants.FirstOrDefault(p => p.SteamID == _playerInClutch1.SteamId);
                if (player1 == null)
                {
                    return;
                }
                ...
                DemoInfoPlayer player2 = Parser.PlayingParticipants.FirstOrDefault(p => p.Team.ToSide() == player2Team);
                if (player2 == null)
                {
                    return;
                }
        ...
        protected void CheckForSpecialClutchEnd()
        {
            ...
            DemoInfoPlayer pl = Parser.PlayingParticipants.FirstOrDefault(p => p.SteamID == _playerInClutch1.SteamId);
            if (pl == null)
            {
                return;
            }
        ...
        {
            foreach (DemoInfoPlayer participant in Parser.Participants)
            {
                if (participant.SteamID == 0 || participant.Team == DemoInfoTeam.Spectate)
                {
                    continue;
                }
                ...
                {
                    case DemoInfoTeam.CounterTerrorist when !Demo.TeamCT.Players.Contains(player):
                        Demo.TeamCT.Players.Add(player);
                        player.TeamName = Demo.TeamCT.Name;
                        break;
                    case DemoInfoTeam.Terrorist when !Demo.TeamT.Players.Contains(player):
