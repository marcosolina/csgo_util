...
public class DemFileParserMarco implements DemFileParser {
	...
	public boolean processFiles() throws MarcoException {
		...
		List<File> filesToProcess = filesEntities.stream().map(e -> new File(e.getFileName())).collect(Collectors.toList());
	...
	private void processFiles(List<File> files) {
		...
		files.stream().forEach(f -> {
			try {
				...
				if (m.getUsersStats() != null) {
					...
					count.incrementAndGet();
					f.delete();
					setFileProcessed(f, DemProcessStatus.DELETED);
				}
		...
		notificationService.sendParsingCompleteNotification("Dem Manager", String.format("Processed %d files, %d were successfully processed", files.size(), count.get()));
	}
	...
	@Override
	public Map<String, List<MapStats>> getUsersStatsForLastXGames(Integer gamesCounter, List<String> usersIDs, BigDecimal minPercPlayed) throws MarcoException {
