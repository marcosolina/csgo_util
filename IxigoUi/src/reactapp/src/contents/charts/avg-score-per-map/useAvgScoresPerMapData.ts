...
import { useGetAvgScoresPerMap } from "../../../services/charts";
...
export const useAvgScoresPerMapData = (request: IUseAvgScoresPerMapDataRequest): IUseAvgScoresPerMapDataResult => {
  const qAvgScores = useGetAvgScoresPerMap({ steamIds: request.steamIds, scoreType: request.scoreType });
  const qCsgoPlayers = useGetCsgoPlayers();
  const qScoreTypes = useGetScoreTypes();

  const status = combineQueryStatuses([qAvgScores, qCsgoPlayers, qScoreTypes]);
