...
import { useAvgScoresPerMapData } from "./usePlayersAvgScoresPerMapData";
...
const fillMissingMaps = (
  allMaps: string[],
  avgScores: IAvgPlayerScorePerMap[],
  steamId: string
): IAvgPlayerScorePerMap[] => {
  const filledArray = allMaps.map((mapName): IAvgPlayerScorePerMap => {
...
const PlayersAvgScorePerMap = () => {
  ...
  return (
    <>
      <Typography align="center">{t("page.charts.avgPlayersScorePerMap.title")}</Typography>
      <Grid container spacing={DEFAULT_SPACING} padding={DEFAULT_SPACING}>
        <Grid item xs={XS} sm={SM} md={MD} lg={LG} xl={XL}>
          <IxigoSelect
            label={t("page.charts.avgPlayersScorePerMap.lblScoreTypes") as string}
        ...
        <Grid item xs={XS} sm={SM} md={MD} lg={LG} xl={XL}>
          <IxigoSelectMultiple
            label={t("page.charts.avgPlayersScorePerMap.lblDpUsers") as string}
        ...
        <Grid item xs={XS} sm={SM} md={MD} lg={LG} xl={XL}>
          <IxigoSelectMultiple
            label={t("page.charts.avgPlayersScorePerMap.lblDpMaps") as string}
            selectedValues={scoresForMaps}
            possibleValues={mapsPlayed.map((m) => ({ value: m.map_name, label: m.map_name }))}
            onChange={setScoresForMaps}
            helperText={t("page.charts.avgPlayersScorePerMap.txtHelpDpMaps") as string}
          />
        </Grid>
        <Grid item xs={XS} sm={SM} md={MD} lg={LG} xl={XL}>
          <IxigoText
            label={t("page.charts.avgPlayersScorePerMap.lblMatchCount") as string}
            type={IxigoTextType.number}
            value={matchesToConsider}
            onChange={setMatchesToConsider}
            helperText={t("page.charts.avgPlayersScorePerMap.txtHelpMatchCount") as string}
          />
        </Grid>
      </Grid>
      <Switch value={status}>
...
export default PlayersAvgScorePerMap;
