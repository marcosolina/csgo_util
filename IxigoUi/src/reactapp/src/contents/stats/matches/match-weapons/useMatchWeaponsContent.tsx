...
const COLUMNS_ORDER: string[] = [
  ...
  function createColumnDefinition(
  ...
  ): MRT_ColumnDef<IWeaponMatchData> {
    const cell: MRT_ColumnDef<IWeaponMatchData> = {
      id: key,
      accessorFn: (row) => row[key as keyof IWeaponMatchData],
      header: t(`${COL_HEADERS_BASE_TRANSLATION_KEY}.${key}.header`),
      size: SMALL_COL_SIZE,
      Header: customHeader<IWeaponMatchData>(t(`${COL_HEADERS_BASE_TRANSLATION_KEY}.${key}.tooltip`)),
    };
  
    if (key === "username") {
      cell.Cell = ({ cell }: { cell: MRT_Cell<IWeaponMatchData> }) => {
    ...
    if (key === "accuracy" || key === "headshotkills_percentage" || key === "headshot_percentage" || key === "chest_hit_percentage" || key === "stomach_hit_percentage" || key === "arm_hit_percentage" || key === "leg_hit_percentage") {
        cell.Cell = ({ cell }: { cell: MRT_Cell<IWeaponMatchData> }) => {
...
export const useMatchWeaponsContent = (request: IMatchWeaponContentRequest): IMatchWeaponContent => {
    ...
    const [data, setData] = useState<IWeaponMatchData[]>([]);
    ...
    // Create the columns
    const columns = useMemo<MRT_ColumnDef<IWeaponMatchData>[]>(() => {
      const cols: MRT_ColumnDef<IWeaponMatchData>[] = [];
    ...
    useEffect(() => {
      if (queriesState === QueryStatus.success) {
        ...
        matchPlayerWeaponStats = matchPlayerWeaponStats.map((wd: IWeaponMatchData) => ({
          ...wd,
          weapon_img: wd.weapon,
        }));

        const data: IWeaponMatchData[] = [];
        matchPlayerWeaponStats.forEach((playerStat) => {
          const user = users.find(
            (user) => user.steam_id === playerStat.steamid
          );
          if (!user) {
            return;
          }
