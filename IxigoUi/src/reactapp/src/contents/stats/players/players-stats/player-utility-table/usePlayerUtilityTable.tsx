import { useEffect, useMemo, useState } from "react";
import { IPlayerOverallStats, PLAYER_OVERALL_STATS_REQUEST, useGetStats } from "../../../../../services/stats";
import { QueryStatus } from "../../../../../lib/http-requests";
import { TFunction } from "i18next";
import { MRT_ColumnDef } from "material-react-table";
import customHeader from "../../../../../common/material-table/custom-header/customHeader";
import { useTranslation } from "react-i18next";
import { IPlayerUtilityTableRequest, IPlayerUtilityTableResponse, IUtilityData } from "./interfaces";

const COL_HEADERS_BASE_TRANSLATION_KEY = "page.stats.player.content.overall.utility.column-headers";
const SMALL_COL_SIZE = 5;

const COLUMNS_ORDER: string[] = ["desc", "value"];

function createColumnDefinition(
  key: string,
  t: TFunction<"translation", undefined, "translation">
): MRT_ColumnDef<IUtilityData> {
  const cell: MRT_ColumnDef<IUtilityData> = {
    id: key,
    accessorFn: (row) => row[key as keyof IUtilityData],
    header: t(`${COL_HEADERS_BASE_TRANSLATION_KEY}.${key}.header`),
    size: SMALL_COL_SIZE,
    Header: customHeader<IUtilityData>(t(`${COL_HEADERS_BASE_TRANSLATION_KEY}.${key}.tooltip`)),
  };

  return cell;
}

export function usePlayerUtilityTable(request: IPlayerUtilityTableRequest): IPlayerUtilityTableResponse {
  const { t } = useTranslation();
  const [data, setData] = useState<IUtilityData[]>([]);
  const [playerOverall, setPlayerOverall] = useState<IPlayerOverallStats>();
  const [maxUD, setMaxUD] = useState<number>(0);
  const qUsersRequest = useGetStats(PLAYER_OVERALL_STATS_REQUEST);

  // Create the columns
  const columns = useMemo<MRT_ColumnDef<IUtilityData>[]>(() => {
    const cols: MRT_ColumnDef<IUtilityData>[] = [];
    COLUMNS_ORDER.forEach((key) => {
      cols.push(createColumnDefinition(key, t));
    });
    return cols;
  }, [t]);

  useEffect(() => {
    if (qUsersRequest.status === QueryStatus.success && qUsersRequest.data?.data?.view_data.length) {
      const userData = qUsersRequest.data.data.view_data.find((p) => p.steamid === request.steamid);
      if (!userData) return;

      const playerData: IUtilityData[] = [];
      playerData.push({
        desc: `${t(`${COL_HEADERS_BASE_TRANSLATION_KEY}.row1.label`)}`,
        value: `${userData.fa.toFixed(0)}`,
      });
      playerData.push({
        desc: `${t(`${COL_HEADERS_BASE_TRANSLATION_KEY}.row2.label`)}`,
        value: `${userData.ebt.toFixed(2)}s`,
      });
      playerData.push({
        desc: `${t(`${COL_HEADERS_BASE_TRANSLATION_KEY}.row3.label`)}`,
        value: `${userData.fbt.toFixed(2)}s`,
      });
      setData(playerData);
      setPlayerOverall(userData);
      setMaxUD(Math.max(...qUsersRequest.data.data.view_data.map((p) => p.ud)));
    }
  }, [qUsersRequest.status, qUsersRequest.data, request.steamid, t]);

  return {
    state: qUsersRequest.status,
    data,
    columns,
    maxUD,
    playerOverall,
  };
}
